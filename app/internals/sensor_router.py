...
async def display_sensor_data(data: SensorData):
    sensorData_collection = mongo_db_connector.init_sensor_datadb("temperature_humidity")
    sensorData_collection.insert_one(data.model_dump())
    return {"message": "Temperature and humidity data successfully added"}

@internal.get("/dashboard")
async def display_sensor_data(request: Request):
